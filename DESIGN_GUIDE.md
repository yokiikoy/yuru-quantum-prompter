...
## 8. 溢れ防止仕様（NO-OVERFLOW SPEC）

プレゼン中にスクロールが発生すると没入感が削がれるため、以下の制約を厳守してください。

| 項目 | 制限内容 | 備考 |
|---|---|---|
| **基本文字数** | 1枚あたり **最大150文字** 程度 | 画像ありの場合は **100文字** 以内 |
| **画像あり** | テキストは2〜3段落、またはバブル1つまで | 画像とテキストが重ならないように調整 |
| **バブル数** | 1画面につき **最大3つ** まで | 2カラムなら2段、1カラムなら3つが限度 |
| **引用（長文）** | 3行を超える場合は分割するか、要約する | 台本（script）には全文を残し、スライドは要約 |
| **コード/数式** | 縦幅を取るため、前後のテキストを極限まで削る | - |

### 溢れそうな時の対処法：
1.  **スライドを分割する**: 1枚に詰め込まず、rangeを細かく分けて2枚にする。
2.  **内容をscriptへ逃がす**: スライド（content）は「結論」だけにし、説明はスライド下の台本エリアに任せる。
3.  **圧縮クラスの利用**: デザインガイドの「パターンB（圧縮版）」など、スクロールを回避するパターンを manifest.json 内で指定する。
...
を参照し、特に「8. 溢れ防止仕様」に基づく厳格な文字数・レイアウト管理を行ってください。
